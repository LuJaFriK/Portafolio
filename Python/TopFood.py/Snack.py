...
class Snack(Alimento):
    def __init__(self, nombre: str, costo: float, comentario: str, existencia: bool, porcion: int) -> None:
        super().__init__(nombre, costo, comentario, existencia)
        self.individual = True
        self.porcion_cantidad = porcion

    def get_costo(self) -> float:
        new_costo: float = super().get_costo()
        if (not self.individual):
            new_costo *= (self.porcion_cantidad-2)
        return new_costo

    def alternar_individual(self,valor: bool) -> None:
